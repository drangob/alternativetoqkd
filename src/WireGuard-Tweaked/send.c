...
void state_message_send(struct wireguard_peer *peer) {
	struct state_data packet;
	state_message_create(&packet, 0, 1, 2);
	socket_send_buffer_to_peer(peer, &packet, sizeof(struct state_data), 0);
	printk(KERN_CRIT "sent a packet of state!");
}
