...
int main(int argc, char const *argv[]) {
	...
	uint32_t len= 250;
	unsigned char associatedData[] = "\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF";
	uint32_t associatedDataLength = 12;
	int outputLen;
	unsigned char output[500];
	AEAD_AES_128_CBC_HMAC_SHA_256_ENCRYPT(key, inputString, len, associatedData, associatedDataLength, output, &outputLen);
	...
	AEAD_AES_128_CBC_HMAC_SHA_256_DECRYPT(key, output, outputLen, associatedData, associatedDataLength, finalOutput, &finalOutLen);
